...
# URL
@export var DEFAULT_PAGE = "user://default_page.html"
const DRAG_DROP_PAGE = "user://dragdrop_page.html"
@export var HOME_PAGE = "https://github.com/Lecrapouille/gdcef"
...
# not found.
# ==============================================================================
func get_browser(index): # Теперь принимаем int
	if not $CEF.is_alive():
		return null
    
    # Проверяем, существует ли ребенок с таким порядковым номером
	if index >= 0 and index < $CEF.get_child_count():
		return $CEF.get_child(index)
    
	$Panel/VBox/BottomBar/Info.set_text("Browser at index " + str(index) + " not found")
	return null
...
func _on_Add_pressed():
	var browser = await create_browser(ProjectSettings.globalize_path(DEFAULT_PAGE))
	if browser != null:
...
func _on_BrowserList_item_selected(index):
	current_browser = get_browser(index) # Убрали str()
	if current_browser != null:
